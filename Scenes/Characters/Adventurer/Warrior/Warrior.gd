...
@onready var BlueSlime = Global.BLUESLIMEDATA
@onready var Warrior = Global.WARRIORDATA
...
func DetectionBoxEntered(body):
	if body.is_in_group("BlueSlime") or body.is_in_group("RedSlime"):
...
func DamageBoxEntered(area):
	if area.is_in_group("BlueSlimeHitbox"):
		print(area.get_parent().get_parent().Stats.health)
...
func HitboxEntered(area):
	...
	if area.is_in_group("BlueSlimeDamageBox"):
		if damageTaken == false and dead == false:
			Stats.health -= BlueSlime.meleeDamage
